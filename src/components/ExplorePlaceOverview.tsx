...
type ExplorePlaceOverviewProps = {
  place: ExplorePlace;
  compact?: boolean;
};

export default function ExplorePlaceOverview({
  place,
  compact = false,
}: ExplorePlaceOverviewProps) {
      ...
      <div>
        ...
        <p
          className={`leading-relaxed text-slate-600 ${
            compact ? "text-sm" : "mt-1.5 text-sm"
          }`}
        >
          {place.description}
        </p>
...
function PinIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-3 w-3 text-miami-coral" aria-hidden>
...
function ShieldIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="mt-0.5 h-4 w-4 shrink-0 text-miami-teal" aria-hidden>
